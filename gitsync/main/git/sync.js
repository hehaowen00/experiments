...
function register({ mainWindow, git, gitRaw, gitProgress }) {
  ipcMain.handle('git:pull', async (_, repoPath, strategy, remote) => {
    try {
      const args = ['pull', '--autostash', '--progress'];
      ...
      const out = await gitProgress(repoPath, args);
  ...
  ipcMain.handle('git:pushForce', async (_, repoPath, remote) => {
    try {
      const args = ['push', '--force-with-lease', '--progress'];
      if (remote) args.push(remote);
      const out = await gitProgress(repoPath, args);
      return { ok: true, output: out };
    } catch (e) {
  ...
  ipcMain.handle('git:pushSetUpstream', async (_, repoPath, remote, branch) => {
    try {
      const out = await gitProgress(repoPath, [
        'push',
        '-u',
        '--progress',
        remote,
