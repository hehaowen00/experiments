...
function register(mainWindow) {
  ...
  ipcMain.handle('dbConn:getQueries', (_, id) => {
    const row = store.getDb().prepare('SELECT queries FROM db_connections WHERE id = ?').get(id);
    if (!row) return [];
    try {
      return JSON.parse(row.queries || '[]');
    } catch {
      return [];
    }
  });

  ipcMain.handle('dbConn:saveQueries', (_, id, queries) => {
    store.getDb()
      .prepare('UPDATE db_connections SET queries = ? WHERE id = ?')
      .run(JSON.stringify(queries || []), id);
    return { ok: true };
  });
